...
class SignalClientTests: XCTestCase {
    ...
    func testFetchAttachment() throws {
    ...
    func testSignatureValidation() {
        let completed=XCTestExpectation()
        let example = SignatureValidator()
        ...
        return when(fulfilled: files)
            .then { downloads -> Promise<[AttachmentInfo]> in
                ...
                if try SignalCommonCrypto.random(bytes: 1)[0] < 80 {
                    uploads.append(self.forsta.signal.uploadAttachment(data: Data(base64Encoded: Conversation.self.memeData)!,
                                                                       name: "meme.jpg", type: "image/jpeg", mtime: Date()))
                }
...
class SignatureValidator: SignalClientDelegate {
    let forsta1 = try! ForstaClient(MemoryKVStore())
    let forsta2 = try! ForstaClient(MemoryKVStore())
    let (finished, finishedSeal) = Promise<Void>.pending()
    ...
    func go() -> Promise<Void> {
        ...
        return firstly {
            forsta1.atlas.authenticateViaPassword(userTag: "greg1", password: "asdfasdf24")
        }
        .map { user in
            distribution = "<\(user["tag"]["id"].stringValue)>"
        }
        .map {
            self.forsta1.signal.registerDevice(deviceLabel: "test new device signature stuff")
        }
        .then { task in
            task.complete
        }
        .map {
            self.forsta1.signal.delegates.add(self)
            try self.forsta1.connect()
        }
        .then {
            self.forsta2.atlas.authenticateViaPassword(userTag: "greg1", password: "asdfasdf24")
        }
        .map { _ in
            self.forsta2.signal.registerDevice(deviceLabel: "test new device signature stuff")
        }
        .then { task in
            task.complete
        }
        .map {
            self.forsta2.signal.delegates.add(self)
            try self.forsta2.connect()
        }
        .then { _ -> Promise<[AttachmentInfo]> in
            var uploads = [Promise<AttachmentInfo>]()
            for i in 1...((try SignalCommonCrypto.random(bytes: 1)[0] % 3) + 2) {
                uploads.append(self.forsta1.signal.uploadAttachment(data: Data(base64Encoded: Conversation.memeData)!,
                                                                    name: "meme\(i).jpg", type: "image/jpeg", mtime: Date()))
            }
            return when(fulfilled: uploads)
        }
    ...
    func inboundMessage(message: InboundMessage) {
        if message.payload.messageType! == .content {
            let text = message.payload.bodyPlain ?? ""
            let validated = (try? message.verifySignature(store: forsta2.signal.store)) ?? false
            
            print("Received text: \(text)")
            print("Message content is valid: \(validated)")
            ...
            if validated {
                ...
                finishedSeal.reject(ForstaError(.invalidSignature))
            }
        }
    }
}
