...
class AtlasClient {
            ...
            .map { result in
                ...
                if statusCode == 200 {
                    ...
                    if self.setJwt(json["token"].stringValue) {
                        ...
                        throw LibRelayError.internalError(why: "malformed or expired jwt")
                    }
                } else {
                    throw LibRelayError.requestRejected(why: json)
                }
            ...
            .map { result in
                ...
                if statusCode == 200 {
                    if let jwt = json["token"].string {
                        ...
                        throw LibRelayError.internalError(why: "malformed refresh response")
                    }
                } else {
                    throw LibRelayError.requestRejected(why: json)
                }
        ...
        return request("/v1/join/\(invitationToken ?? "")", method: .post, parameters: fields)
            .map { result in
                ...
                if statusCode == 200,
                    ...
                    if (!self.setJwt(jwt)) {
                        throw LibRelayError.internalError(why: "user creation succeeded but setJwt failed")
                    }
        ...
        return resolveTagExpressionBatch([expression])
            .map { result in
                ...
                if res.count > 0 {
                    ...
                    throw LibRelayError.internalError(why: "malformed tagmath resolution response")
                }
        }
    }
    ...
    func getUsers(userIds: [String], onlyPublicDirectory: Bool = false) -> Promise<[JSON]> {
        ...
        return (onlyPublicDirectory ? Promise.value((200, JSON(["result": []]))) : request("/v1/user/?id_in=" + userIds.joined(separator: ",")))
            .map { result in
                let (statusCode, json) = result
                if (statusCode != 200) { throw LibRelayError.requestRejected(why: json) }
